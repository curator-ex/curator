defmodule Mix.Tasks.Curator.Registerable.Install do
  ...
  def run(args) do
    ...
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix curator.registerable.install can only be run inside an application directory"
      )
    end
  ...
  def files_to_be_generated(%Context{schema: schema, context_app: context_app}) do
    ...
    [
      {:eex, "registration_controller.ex",
       Path.join([web_prefix, "controllers", web_path, "auth", "registration_controller.ex"])},
      {:eex, "registerable.ex", Path.join([web_prefix, web_path, "auth", "registerable.ex"])},
      {:eex, "edit.html.eex",
       Path.join([web_prefix, "templates", web_path, "auth", "registration", "edit.html.eex"])},
      {:eex, "form.html.eex",
       Path.join([web_prefix, "templates", web_path, "auth", "registration", "form.html.eex"])},
      {:eex, "new.html.eex",
       Path.join([web_prefix, "templates", web_path, "auth", "registration", "new.html.eex"])},
      {:eex, "show.html.eex",
       Path.join([web_prefix, "templates", web_path, "auth", "registration", "show.html.eex"])},
      {:eex, "view.ex",
       Path.join([web_prefix, "views", web_path, "auth", "registration_view.ex"])}
    ]
  end
  ...
  def print_shell_instructions(%Context{schema: schema, context_app: context_app} = context) do
    ...
    The Registerable module was created at: #{
      Path.join([web_prefix, web_path, "auth", "registerable.ex"])
    }
