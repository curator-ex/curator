defmodule Mix.Tasks.Curator.Approvable.Install do
  @shortdoc "Install` Curator"

  @moduledoc """
  Generates required Curator Approvable files.
  ...
  def run(args) do
    ...
    if Mix.Project.umbrella? do
      Mix.raise "mix curator.approvable.install can only be run inside an application directory"
    end
  ...
  def files_to_be_generated(%Context{schema: schema, context_app: context_app}) do
    ...
    [
      {:eex,     "approvable.ex",              Path.join([web_prefix, web_path, "auth", "approvable.ex"])},
      # {:eex,     "approvable_controller.ex",   Path.join([web_prefix, "controllers", web_path, "auth", "approvable_controller.ex"])},
      {:eex,     "migration.exs",               Path.join([db_prefix, "priv/repo/migrations/#{timestamp()}_add_approvable_to_users.exs"])},
      {:eex,     "email/approvable.html.eex",  Path.join([web_prefix, "templates", web_path, "auth", "email", "approvable.html.eex"])},
    ]
  end
  ...
  def copy_new_files(%Context{} = context, paths, binding) do
    ...
    Mix.Phoenix.copy_from paths, "priv/templates/curator.approvable.install", binding, files
  ...
  defp inject_email_module(context, paths, binding) do
    paths
    |> Mix.Phoenix.eval_from("priv/templates/curator.approvable.install/email.ex", binding)
  ...
  def print_shell_instructions(%Context{schema: schema, context_app: context_app} = context) do
    ...
    The Approvable module was created at: #{Path.join([web_prefix, web_path, "auth", "approvable.ex"])}
    ...
    Be sure to add it to Curator: #{Path.join([web_prefix, web_path, "auth", "curator.ex"])}
        ...
        use Curator,
          modules: [#{inspect context.web_module}.Auth.Approvable]
