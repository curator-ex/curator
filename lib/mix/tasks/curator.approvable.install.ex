defmodule Mix.Tasks.Curator.Approvable.Install do
  ...
  def files_to_be_generated(%Context{schema: schema, context_app: context_app}) do
    ...
    [
      ...
      {:eex, "migration.exs",
       Path.join([db_prefix, "priv/repo/migrations/#{timestamp()}_add_approvable_to_users.exs"])},
      {:eex, "email/approvable.html.eex",
       Path.join([web_prefix, "templates", web_path, "auth", "email", "approvable.html.eex"])}
    ]
  end
  ...
  def print_shell_instructions(%Context{schema: schema, context_app: context_app} = context) do
    ...
    The Approvable module was created at: #{
      Path.join([web_prefix, web_path, "auth", "approvable.ex"])
    }
